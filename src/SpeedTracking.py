...
async def detect_speed(video_file: UploadFile = File(...), speed_limit: float = Form(...)):
    try:
        ...
        # Lưu trữ cả tọa độ x và y để tính toán tốc độ chính xác hơn
        ...
        with sv.VideoSink(output_path, video_info) as sink:
            for frame in frame_generator:
                ...
                for i in range(len(detections.tracker_id)):
                    ...
                    # Yêu cầu ít nhất nửa giây dữ liệu để tính toán tốc độ
                    if len(coordinates[tracker_id]) < video_info.fps / 2:
                        labels.append(f"#{tracker_id}")
                        colors.append((0, 255, 0))
                    else:
                        # Tính khoảng cách Euclidean giữa điểm đầu và điểm cuối
                        start_point = coordinates[tracker_id][0]
                        end_point = coordinates[tracker_id][-1]
                        
                        # Tính toán khoảng cách sử dụng cả x và y
                        distance = np.sqrt(
                            (end_point[0] - start_point[0])**2 + 
                            (end_point[1] - start_point[1])**2
                        )
                        
                        # Tính thời gian giữa hai điểm
                        time_elapsed = time_records[tracker_id][-1] - time_records[tracker_id][0]
                        
                        # Tránh chia cho 0
                        if time_elapsed > 0:
                            # Áp dụng hệ số và chuyển đổi sang km/h
                            speed = (distance / time_elapsed) * 3.6
                            
                            # Giới hạn tốc độ tối đa ở mức hợp lý (150 km/h)
                            speed = min(speed, 150.0)
                            
                            labels.append(f"#{tracker_id} {int(speed)} km/h")

                            # Cập nhật thông tin xe nếu chưa có hoặc tốc độ cao hơn
        ...
        vehicles_list = list(processed_vehicles.values())
        violators_list = list(processed_violators.values())
