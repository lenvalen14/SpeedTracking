...
async def detect_speed(video_file: UploadFile = File(...), speed_limit: float = Form(...)):
    ...
    with sv.VideoSink(output_path, video_info) as sink:
        for frame in frame_generator:
            ...
            labels, colors = [], []
            for detection, tracker_id, box in zip(detections, detections.tracker_id, detections.xyxy):
    ...
    return JSONResponse(content={
        "violators": violators,
        "output_video": output_path  # Ensure this is properly handled in frontend
    })